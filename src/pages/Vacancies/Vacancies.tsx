...
const Vacancies = () => {
  ...
  useEffect(() => {
    const searchInput: string | undefined =
      search.length !== 0 ? search : undefined;
    const selectedCity: string | undefined =
      selectedItem.length !== 0 ? selectedItem : undefined;
    ...
    let graphics: string[] | undefined = Object.keys(graphic).filter(
      (graphicKey) => {
    ...
    const proffs = isProfessions.map((isProfession, i) =>
      isProfession ? professions[i].id : null
    );
  ...
  const paginate = () => {
    if (inView && vacancies.length >= 10 && vacancies.length % 10 === 0) {
      const currentPage = vacancies.length / 10;
  ...
  const handleProfessionChange = (index: number) => () => {
    const newProfession = isProfessions.map((isProfession, i) =>
      i === index ? !isProfession : isProfession
    );
  ...
  const handleGraphicChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    const copyConfig = {
      ...graphic,
      [e.target.id]: !graphic[e.target.id as keyof IGraphic],
    };
